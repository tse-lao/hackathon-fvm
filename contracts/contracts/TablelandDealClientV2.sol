...
contract TablelandDealClientV2 is AxelarExecutable, IDealClient {
    ...
    function _execute(bytes calldata payload_) internal {
        ...
        uint64 car_size;
        (piece_cid,label,piece_size,end_epoch,sender,location_ref,car_size) = abi.decode(payload_,(bytes, string, uint64, int64, address, string, uint64));
        ExtraParamsV1 memory extraParams = ExtraParamsV1(location_ref,car_size,false,false);

        DealRequest memory deal = DealRequest(piece_cid,piece_size,false,label,int64(uint64((block.number + 172800))),end_epoch,storage_price_per_epoch,0,0,1,extraParams);
    ...
    function makeDealProposal(
        ...
        require(
            currentPieceRequestInfo[deal.piece_cid].status ==
                ...
                currentPieceRequestInfo[deal.piece_cid].status ==
                Status.DealTerminated || currentPieceRequestInfo[deal.piece_cid].status ==
    ...
    function updateActivationStatus(bytes memory pieceCid) internal {
            ...
            DealRequest memory deal = request[currentPieceRequestInfo[pieceCid].currentRequestID];
            if(deal.start_epoch <= int64(uint64(block.number))){
                currentPieceRequestInfo[pieceCid].status = Status
                        .RequestTimeout;
                        idealTablelandView.toUpdateStatus("RequestTimeout",dealid);
            }
